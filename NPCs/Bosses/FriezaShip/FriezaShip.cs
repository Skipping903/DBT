...
namespace DBT.NPCs.Bosses.FriezaShip
{
	...
	public class FriezaShip : ModNPC
	{
				...
				if (AITimer > 0)
				{
					...
					for (int k = 0; k < 10; k++)
					{
						if (Deg <= 360)
						{
							...
							float CPosX = npc.Center.X + ShieldDistance * (float)Math.Cos(Deg); //To find the circumference you use formula: x = cX + r * cos(angle), where the x is the coordinate, cX is the center of the circle by X and r is radius.
					...
					{
						player.Hurt(PlayerDeathReason.ByCustomReason(player.name + "has been cut in half by the Frieza Force Shield"), 40, 1);
						Dust dust = Main.dust[Dust.NewDust(player.Center, player.width, player.height, 56)];//need to pick a new dust
						dust.noGravity = true;
					...
					for (int i = 0; i <= Main.maxProjectiles; i++)
					{
						...
						if (Vector2.Distance(projectile.Center, npc.Center) <= ShieldDistance && projectile.active)
						{
							...
							for (int j = 0; j < 20; j++)
							{
								Dust dust = Main.dust[Dust.NewDust(projectile.position, projectile.width, projectile.height, 211)];
								dust.noGravity = true;
							}
							...
							if (projectile.Center.X > player.Center.X * 0.5f)//Reference goes to Fargo for voring his epic code.
							{
		...
		private void Slam()
		{
			if (IterationCount < PlayerCount().Count)//Since the list length is 1, but to call the first member we need 0, so it is +1;
			{
				if (AITimer < 130)
				{
					DustScaleTimer++;
